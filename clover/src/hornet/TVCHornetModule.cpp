...
void TVCHornetModule::step_control_loop(DataPacket& data )
{
...
std::pair<TVCHornetStateOutput, TVCFlightData> TVCHornetModule::flight_tick(const AnalogSensorReadings& analog_sensors, FlightStateOutput& flight_output)
{
    TVCHornetStateOutput out{};
    TVCFlightData data{};
...
std::expected<void, Error> TVCHornetModule::load_sequence(const TVCLoadSequenceRequest& req)
{
    ...
    if (!result_pitch)
        return std::unexpected(result_pitch.error().context("%s", "Invalid X trace"));

    auto result_yaw = yaw_trace.load(req.yaw_angle_trace_deg);
    if (!result_yaw)
        return std::unexpected(result_yaw.error().context("%s", "Invalid Y trace"));
