...
std::expected<std::tuple<float, HornetThrottleMetrics>, Error> HornetThrottle::tick(float thrust_command_N)
{
    ...
    if (thrust_command_N < 44.044){ // below 10 lbf (only ever during landing and takeoff sequences)
        float best_fit_output = (thrust_command_N / 69.911f );
        float throttle_percent = std::clamp(best_fit_output, 0.0, 0.63f);

    } else { // the actual operating range
        float best_fit_output = (thrust_command_N + 32.5271f) / 121.53f;
        float throttle_percent = std::clamp(best_fit_output, 0.63f, 1.0f);
    }
