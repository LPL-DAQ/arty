...
std::expected<void, Error> Trace::load(const ControlTrace& trace)
{
    ...
    for (int i = 0; i < trace.segments_count; i++) {
        ...
        case Segment_sine_tag:
            ...
            if (seg.type.sine.period_ms <= 0.0f) {
                return std::unexpected(Error::from_cause("period for sine segment %d must be positive, got %f", i+1, static_cast<double>(seg.type.sine.period_ms)));
            }
            break;
...
static float sample_segment(const Segment& segment, const float segment_time_ms)
{
    switch (segment.which_type) {
        ...
        case Segment_sine_tag: {
            ...
            return sine.offset + sine.amplitude * std::sin(segment_time_ms / sine.period_ms * TAU + sine.phase_deg / 360.0f * TAU);
        }
