...
void StateValveSeq::init(bool has_fuel_trace,bool has_lox_trace, float fuel_total_time, float lox_total_time) {
    ...
    StateValveSeq::has_fuel = has_fuel_trace;
    StateValveSeq::has_lox = has_lox_trace;
    StateValveSeq::fuel_total_time = fuel_total_time;
    StateValveSeq::lox_total_time = lox_total_time;
}
...
std::pair<ControllerOutput, ValveSequenceData> StateValveSeq::tick(uint32_t current_time, uint32_t start_time) {
    ...
    if (((has_fuel && has_lox) && (dt >= lox_total_time && dt >= fuel_total_time)) ||
        ((has_fuel && ! has_lox) && dt >= fuel_total_time) ||
