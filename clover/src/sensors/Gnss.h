...
{
    if (id[0] == '~' && id[1] == '~') {
        decode_RT(body, body_length);
        return;
    }
    if (id[0] == 'S' && id[1] == 'T') {
        decode_ST(body, body_length);
        return;
    }
    if (id[0] == 'm' && id[1] == 'p') {
        decode_mp(body, body_length);
        return;
    }
    if (id[0] == 'V' && id[1] == 'E') {
        decode_VE(body, body_length);
        return;
    }
    if (id[0] == 'S' && id[1] == 'G') {
        decode_SG(body, body_length);
        return;
    }
}
...
{
    uint8_t res = 0;
    auto rot_left = [](uint8_t val) -> uint8_t { return (uint8_t)((val << 2) | (val >> 6)); };
                ...
                if ((byte >= '0' && byte <= '9') || (byte >= 'A' && byte <= 'F') || (byte >= 'a' && byte <= 'f')) {
                    ...
                    if (length_count == 3) {
                        ...
                        if (body_length > 0 && body_length <= MAX_BODY_LENGTH) {
                            ...
                            LOG_WRN("[Gnss] invalid body length %d for id %c%c", body_length, id[0], id[1]);
                            state = SYNC;
                        }
                    }
                }
            ...
            case BODY:
                ...
                if (body_count == body_length) {
                    ...
                    if (calc_cs != expected_cs) {
                        LOG_WRN("[Gnss] [%c%c] bad checksum (calc 0x%02X, got 0x%02X)", id[0], id[1], calc_cs, expected_cs);
                    }
                    else {
