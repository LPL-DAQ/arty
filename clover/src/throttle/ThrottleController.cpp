...
std::expected<void, Error> ThrottleController::change_state(ThrottleState new_state)
{
    ...
    // TODO: 
    ThrottleImpl::init_state(new_state);
...
void ThrottleController::step_control_loop(DataPacket& data)
{
    ...
    ThrottleImpl::set_throttle_actuator_data_tag(data);
    data.throttle_state_output = out;

    auto impl_ret = ThrottleImpl::tick(out, data);
    if (!impl_ret.has_value()) {
        LOG_ERR("ThrottleImpl error: %s", impl_ret.error().build_message().c_str());
...
std::expected<void, Error> ThrottleController::handle_load_valve_sequence(const ThrottleLoadValveSequenceRequest& req)
{
    ...
    auto load_ret = ThrottleImpl::load_valve_sequence(req);
    if (!load_ret.has_value()) {
        return std::unexpected(load_ret.error().context("Failed to load valve sequence"));
    }
...
// TOOD: how does this interface with the main system state?
...
std::expected<void, Error> ThrottleController::handle_reset_valve_position(const ThrottleResetValvePositionRequest& req)
{
    ...
    auto ret = ThrottleImpl::reset_valve_position(req.valve, req.new_pos_deg);
    if (!ret.has_value()) {
        return std::unexpected(ret.error().context("Failed to reset valve position"));
    }
    #endif
    return {};
}
