...
public:
    ...
    // Request handlers
    static std::expected<void, Error> load_motor_sequence(const RCSLoadValveSequenceRequest& req);
    static std::expected<void, Error> load_roll_sequence(const RCSLoadRollSequenceRequest& req);
    static std::expected<void, Error> start_motor_sequence();
    ...
    RCSRangerModule() = delete;  // Explicitly prevent instantiation

private:
    static std::pair<RCSRangerStateOutput, RCSIdleData> idle_tick();
    static std::pair<RCSRangerStateOutput, RCSValveSequenceData> motor_sequence_tick(int64_t current_time, int64_t start_time);
    static std::pair<RCSRangerStateOutput, RCSRollSequenceData> roll_sequence_tick(const AnalogSensorReadings& analog_sensors, int64_t current_time, int64_t start_time);
