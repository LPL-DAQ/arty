...
bool vn_parse_imu(const char* line, float* ax, float* ay, float* az, float* gx, float* gy, float* gz)
{
...
bool vn_parse_ins(const char* line, double* lat, double* lon, float* alt, float* vn, float* ve, float* vd)
{
    ...
    *alt = strtof(token, nullptr);
    ...
    if (!token)
        return false;
    *vn = strtof(token, nullptr);
    token = strtok(nullptr, ",");
    if (!token)
        return false;
    *ve = strtof(token, nullptr);
    token = strtok(nullptr, "*");
    if (!token)
        return false;
    *vd = strtof(token, nullptr);
    return true;
}
