...
static int ads79xx_channel_setup(const struct device* dev, const struct adc_channel_cfg* channel_cfg)
{
...
static inline uint16_t ads79xx_ctrl_auto1(bool reset_counter, bool map_gpio, bool range_2x, bool powerdown, uint8_t gpio_out)
{
...
static int ads79xx_read_async(const struct device* dev, const struct adc_sequence* sequence, struct k_poll_signal* async)
{
    struct ads79xx_data* data = dev->data;
    int ret;
...
int ads79xx_init(const struct device* dev)
{
    ...
    k_thread_create(
        ...
        ads79xx_acquisition_thread,
        data,
        NULL,
        NULL,
        CONFIG_ADC_ADS79XX_ACQUISITION_THREAD_PRIO,
        0,
        K_NO_WAIT);
...
#define ADS79XX_INIT(t, n, chan, res) \
    ...
    static const struct ads79xx_config ads##t##_cfg_##n = { \
        .spi = SPI_DT_SPEC_GET(DT_INST_ADS79XX(n, t), ADS79XX_SPI_OP, 50), \
        .channels = chan, \
        .resolution = res, \
