...
#ifdef ADC_CONTEXT_USES_KERNEL_TIMER
#define ADC_CONTEXT_INIT_TIMER(_data, _ctx_name) ._ctx_name.timer = Z_TIMER_INITIALIZER(_data._ctx_name.timer, adc_context_on_timer_expired, NULL)
#endif /* ADC_CONTEXT_USES_KERNEL_TIMER */

#define ADC_CONTEXT_INIT_LOCK(_data, _ctx_name) ._ctx_name.lock = Z_SEM_INITIALIZER(_data._ctx_name.lock, 0, 1)

#define ADC_CONTEXT_INIT_SYNC(_data, _ctx_name) ._ctx_name.sync = Z_SEM_INITIALIZER(_data._ctx_name.sync, 0, 1)
...
static inline void adc_context_lock(struct adc_context* ctx, bool asynchronous, struct k_poll_signal* signal)
{
...
static inline void adc_context_start_read(struct adc_context* ctx, const struct adc_sequence* sequence)
{
 ...
 */
static inline void adc_context_on_sampling_done(struct adc_context* ctx, const struct device* dev)
{
    if (ctx->sequence.options) {
        ...
        default: /* ADC_ACTION_CONTINUE */
            if (ctx->sampling_index < ctx->options.extra_samplings) {
                ++ctx->sampling_index;
            }
            else {
                finish = true;
