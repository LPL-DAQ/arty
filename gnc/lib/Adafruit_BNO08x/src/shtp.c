...
LOG_MODULE_REGISTER(shitp, CONFIG_LOG_DEFAULT_LEVEL);
...
static void addAdvertListener(shtp_t *pShtp, uint16_t guid,
    ...
    LOG_INF("Boom registered guid %u", guid);
...
static void callAdvertHandler(shtp_t *pShtp, uint32_t guid,
    ...
    LOG_INF("Calling handlers %u", guid);
...
static void processAdvertisement(shtp_t *pShtp, uint8_t *payload, uint16_t payloadLen) {
    ...
    LOG_INF("Processing advertisement");
...
int shtp_send(void *pInstance,
    ...
    LOG_INF("len: %d, outmaxpayload: %d", len, pShtp->outMaxPayload);

    if (len > pShtp->outMaxPayload) {
        return SH2_ERR_BAD_PARAM;
    }
    LOG_INF("chan: %d, maxchane: %d", channel, SH2_MAX_CHANS);
...
void shtp_service(void *pInstance) {
    LOG_INF("WHATTHTEHUFHUFHUFHUFHUFHUFHUF %p", pInstance);
    k_sleep(K_MSEC(1000));
    shtp_t *pShtp = (shtp_t *) pInstance;
    uint32_t t_us = 0;

    LOG_INF("pshtp %p", pInstance);
    k_sleep(K_MSEC(50));
    ...
    LOG_INF("fdsafdsafdsfas %p", pInstance);
    k_sleep(K_MSEC(50));
    if (len) {
        LOG_INF("fdasfsafda %p", pInstance);
        k_sleep(K_MSEC(50));
