...
LOG_MODULE_REGISTER(sh2, CONFIG_LOG_DEFAULT_LEVEL);
        ...
        if (reportLen == 0) {
            ...
            if (reportId == SENSORHUB_COMMAND_RESP) {
                ...
                if (pSh2->eventCallback) {
                    ...
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig.changeSensitivity = pGetFeatureResp->
                            changeSensitivity;
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig.reportInterval_us = pGetFeatureResp->
                            reportInterval_uS;
...
static int opProcess(sh2_t *pSh2, const sh2_Op_t *pOp) {
    ...
    LOG_INF("returniniginging start");
    k_sleep(K_MSEC(50));
