 ...
 * extmem.c
 *
 * External QSPI Flash Runtime Table System
 *
 * Current Workflow:
 *   1. Firmware is built inside Docker (Zephyr build).
 *   2. sync_debug.sh copies:
 *        - zephyr.elf
 *        - zephyr.hex
 *        - tables.bin
 *      from container to macOS (~/Desktop/ZephyrDebug).
 *   3. upload_with_openocd.sh (running on macOS):
 *        - Starts OpenOCD
 *        - Uses arm-none-eabi-gdb
 *        - Uploads tables.bin into MCU SRAM
 *        - Sets downloader_pending = 1
 *   4. downloader.c (running on MCU):
 *        - Detects debugger attached
 *        - Calls extmem_write_chunk()
 *        - Programs external QSPI flash
 *
 * External Flash Layout (memory-mapped at 0x90000000):
 *   [Header]
 *   [Directory]
 *   [Table Data Region]
 *
 * Notes:
 *   - Tables are generated by generate_tables.py.
 *   - Tables are written at runtime (NOT compile-time linked).
 *   - linker_extmem.ld is optional and currently unused.
 *   - This system supports development-time updates without
 *     requiring CubeIDE external loader.
 *
 * Future:
 *   - ST external loader (.stldr) may be used for production programming.
 *   - Runtime downloader remains useful for debugging and OTA-style updates.
 */
